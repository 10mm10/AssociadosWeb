...
export default function ClientesPage() {
    ...
    const [procuracaoList, setProcuracaoList] = useState<{ id: number; nome_original: string }[]>([]);
    const [selectedLinkType, setSelectedLinkType] = useState('cadastro');
    const [showLinkOptions, setShowLinkOptions] = useState(false);
    const [selectedOption, setSelectedOption] = useState<'cadastro' | 'procuracao' | null>(null);
    const [procuracaoSelecionada, setProcuracaoSelecionada] = useState<string | null>(null);
    ...
    const isProcuracaoSelecionada = pdfSelecionado &&
        pdfSelecionado.nome_arquivo.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().includes('procuracao');
    ...
    const handleClienteSelect = (cliente: Cliente) => {
        ...
        if (cliente.tipo === "juridica") {
            ...
            updatedCliente.representante_celular = cliente.representante_celular || "";
            updatedCliente.representante_email = cliente.representante_email || "";
            updatedCliente.representante_nacionalidade = cliente.representante_nacionalidade || "";
            updatedCliente.representante_estado_civil = cliente.representante_estado_civil || "";
            updatedCliente.representante_profissao = cliente.representante_profissao || "";
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        if (!clientes) {
            errorAlert("Lista de clientes não carregada. Impossível verificar duplicidade.");
            return;
        }
        ...
        console.log("ID do usuário lido do localStorage (userId):", idDoUsuarioLogado);
        if (!idDoUsuarioLogado) {
            errorAlert("ID do usuário logado não encontrado. Por favor, faça login novamente.");
            return;
        }
        ...
        console.log("Valor enviado para o Backend (id_usuario):", dataToSubmit.id_usuario);
        ...
        try {
            ...
            successToast(`Cliente ${isEditing ? "atualizado" : "cadastrado"} com sucesso!`);
    ...
    const handleDelete = async () => {
        ...
        if (!token) {
            ...
            errorAlert('Token de autenticação não encontrado. Você precisa fazer login novamente.', 'Sessão Expirada');
            return;
        }
        // 1. Determinação da Ação e Mensagem (mantida)
        if (selectedPdfId) {
            titulo = "Excluir PDF";
            confirmationMessage = "Tem certeza que deseja excluir este PDF? Esta ação será registrada para auditoria.";
            urlEndpoint = `/pdfs/${selectedPdfId}`;
            successMessage = "PDF excluído com sucesso!";
        } else if (selectedClienteId) {
            titulo = "Excluir Cliente";
            confirmationMessage = "Tem certeza que deseja excluir este cliente e TODOS os seus arquivos? Esta ação será registrada para auditoria.";
        ...
        try {
            ...
            if (response.status === 401 || response.status === 403) {
                ...
                throw new Error("Acesso negado ou sessão expirada. Você foi desconectado.");
            }
            if (!response.ok) {
                const errorData = await response.json();
            ...
            const mensagemErro = err instanceof Error ? err.message : 'Ocorreu um erro desconhecido durante a exclusão.';
    ...
    const handleLimpar = () => {
        ...
        if (userId) {
            ...
            setError("ID do usuário não carregado. Não foi possível atualizar a lista de clientes.");
        }
    };
    ...
    const handleGerarLink = async (tipo: string, procuracaoNome: string | null = null) => {
        if (!selectedClienteId) {
        ...
        try {
            ...
            if (tipo === 'procuracao') {
                ...
                console.log("Variável de Ambiente:", process.env.NEXT_PUBLIC_BACKEND_URL);
                ...
                if (response.data.link) {
                    ...
                    successToast("Link de procuração gerado com sucesso! \n\n" + response.data.link);
                } else {
                    // SUBSTITUÍDO: alert() por errorAlert()
                    errorAlert(response.data.message || "Link não retornado pelo servidor.");
                }
                if (selectedClienteId) {
                    fetchPdfs(selectedClienteId);
                }
    ...
    const handleCopyProcuracaoLink = () => {
        ...
        navigator.clipboard.writeText(linkProcuracao)
            .then(() => {
                // SUBSTITUÍDO: alert() por successToast()
    ...
    const handleEnviarParaZapsign = async (pdfId: string, documentoNome: string) => {
        if (!selectedClienteId) {
            // SUBSTITUÍDO: alert() por errorAlert()
        ...
        try {
            ...
            if (err instanceof Error) {
                ...
                // SUBSTITUÍDO: alert() por errorAlert()
                errorAlert("Erro ao enviar para Zapsign. Verifique o console para mais detalhes.");
            }
            else {
    ...
    const handleSalvarNome = async (pdfId: string) => {
        ...
        if (!tempNome.trim() || (pdfAtual && pdfAtual.nome_arquivo === tempNome.trim())) {
            setEditingPdfId(null);
            return;
        }
        try {
            isSavingRef.current = true; // Trava para impedir que o onBlur execute no mesmo instante
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/renomear-pdf/${pdfId}`, {
                novoNome: tempNome,
            });
            const nomeAtualizado = response.data.novoNome;
            // Atualiza a lista no estado local
            setPdfs((prevPdfs: Pdf[]) =>
                prevPdfs.map((p) => (p.id === pdfId ? { ...p, nome_arquivo: nomeAtualizado } : p))
            );
        } catch (error) {
    ...
    const handleMobileSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    result.error || "Erro ao adicionar cliente."
                );
            }
                        ...
                        <section className="rounded-lg bg-white p-3 shadow-sm">
                            <div className="mb-3 flex items-center justify-between gap-2">
                                ...
                                <button
                                    type="submit"
                                    form="clienteForm"
                                    className="cursor-pointer rounded-md bg-blue-400 px-3 py-2 text-xs font-semibold text-slate-700 transition active:scale-[0.99]"
                                >
                                    Atualizar
                                </button>
                            </div>
                                ...
                                {selectedLinkType === "procuracao" ? (
                                        ...
                                        {procuracaoList.length > 0 ? (
                                            procuracaoList.map(
                                                (procuracao: {
                                                    ...
                                                    <label
                                                        key={procuracao.id}
                                                        className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-3 text-xs transition ${procuracaoSelecionada ===
                                    ...
                                    <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-3">
                                        <p className="text-xs leading-5 text-blue-700">
                                            O link será associado ao cliente selecionado
                                            e poderá ser enviado para ele completar o cadastro.
                                        </p>
                                    </div>
                                )}
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            if (
                                                ...
                                                alert(
                                                    "Selecione uma procuração para continuar."
                                                );
                                            }
                                        }}
                    ...
                    ====================================================== */}
                                ...
                                {tipoCliente === "fisica" ? (
                                    <>
                                        ...
                                        <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="nacionalidade" className="text-xs font-medium text-slate-600">
                                                    Nacionalidade:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="profissao" className="text-xs font-medium text-slate-600">
                                                    Profissão:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="ctps" className="text-xs font-medium text-slate-600">
                                                    CTPS:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="teleitor" className="text-xs font-medium text-slate-600">
                                                    T. Eleitor:
                                                </label>
                                                <input
                                                    type="text"
                                    ...
                                    <>
                                        <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-3">
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="nome_fantasia" className="text-xs font-medium text-slate-600">
                                                    Nome Fantasia:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="inscricao_estadual" className="text-xs font-medium text-slate-600">
                                                    Insc. Estadual:
                                                </label>
                                                <input
                                                    type="text"
                                        ...
                                        <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="cnpj" className="text-xs font-medium text-slate-600">
                                                    CNPJ:
                                                </label>
                                                <IMaskInput
                                                    mask={cnpjMask}
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_nome" className="text-xs font-medium text-slate-600">
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_nacionalidade" className="text-xs font-medium text-slate-600">
                                                    Nacionalidade:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_cpf" className="text-xs font-medium text-slate-600">
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_rg" className="text-xs font-medium text-slate-600">
                                                    RG do Representante:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_profissao" className="text-xs font-medium text-slate-600">
                                                    Profissão:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_estado_civil" className="text-xs font-medium text-slate-600">
                                                    Estado Civil:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_cargo" className="text-xs font-medium text-slate-600">
                                                    Cargo:
                                                </label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_celular" className="text-xs font-medium text-slate-600">
                                            ...
                                            <div className="flex min-w-0 flex-col gap-1">
                                                <label htmlFor="representante_email" className="text-xs font-medium text-slate-600">
                        ...
                        ====================================================== */}
                                <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-5">
                                    <div className="flex min-w-0 flex-col gap-1">
                                        <label htmlFor="cep" className="text-xs font-medium text-slate-600">
                                            CEP:
                                        </label>
                                        <IMaskInput
                                            mask={cepMask}
                                    ...
                                    <div className="flex min-w-0 flex-col gap-1">
                                        <label htmlFor="cidade" className="text-xs font-medium text-slate-600">
                                            Cidade:
                                        </label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="flex min-w-0 flex-col gap-1">
                                        <label htmlFor="bairro" className="text-xs font-medium text-slate-600">
                                            Bairro:
                                        </label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="flex min-w-0 flex-col gap-1">
                                        <label htmlFor="endereco" className="text-xs font-medium text-slate-600">
                                            Endereço:
                                        </label>
                                        <input
                                            type="text"
                        ...
                        ====================================================== */}
                                <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
                                    <div className="flex min-w-0 flex-col gap-1">
                                        <label htmlFor="estado" className="text-xs font-medium text-slate-600">
                                            Estado:
                                        </label>
                                        <select
                                            id="estado"
                        ...
                        ====================================================== */}
                                {tipoCliente === "fisica" && (
                                    <div className="mb-2 grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
                                        <div className="flex min-w-0 flex-col gap-1">
                                            <label htmlFor="estado_civil" className="text-xs font-medium text-slate-600">
                                                Est. civil:
                                            </label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="flex min-w-0 flex-col gap-1">
                                            <label htmlFor="conjuge" className="text-xs font-medium text-slate-600">
                                                Cônjuge:
                                            </label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="flex min-w-0 flex-col gap-1">
                                            <label htmlFor="cpf_conjuge" className="text-xs font-medium text-slate-600">
                                                CPF Cônjuge:
                                            </label>
                                            <IMaskInput
                                        ...
                                        <div className="flex min-w-0 flex-col gap-1">
                                            <label htmlFor="rg_conjuge" className="text-xs font-medium text-slate-600">
                                                RG Cônjuge:
                                            </label>
                                            <input
                                                type="text"
                                                        ...
                                                        clientes.map(
                                                            (cliente, index) => (
                                                                <tr
                                                                    ...
                                                                    className={`cursor-pointer border-b border-slate-100 transition-colors hover:bg-blue-50 ${selectedClienteId ===
                                                                        cliente.id
                                                                        ? "bg-blue-100"
                                                                        : "bg-white"
                                                                        }`}
                                                    ...
                                                    {pdfs.length > 0 ? (
                                                        pdfs.map((pdf) => (
                                                            <tr
                                                                ...
                                                                className={`cursor-pointer border-b border-slate-100 transition-colors hover:bg-blue-50 ${selectedPdfId ===
                                                                    pdf.id
                                                                    ? "bg-blue-100"
                                                                    : "bg-white"
                                                                    }`}
